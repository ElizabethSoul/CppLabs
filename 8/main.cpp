...
public:
    ...
    void setAdr(const std::string& newAdr) {
        if (!newAdr.empty()) {
            adress = newAdr;
        } else {
            std::cerr << "Error: Adress cannot be empty!" << std::endl;
        }
    }
...
int main() {
    ...
    person.setAdr(""); //Адресс не должен быть пустым
